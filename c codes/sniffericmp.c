...
void got_packet(u_char *args, const struct pcap_pkthdr *header,const u_char *packet)
{
    ...
    if (ntohs(eth->ether_type) == 0x0800) { // 0x0800 is IP type
        ...
        if(ip->iph_protocol == IPPROTO_ICMP){ // if the next protocol is icmp protocol then print the IP_SRC, IP_DST, TYPE, CODE
...
int main()
{
    ...
    char filter_exp[] = "ip proto icmp  and (enter two host)";
    bpf_u_int32 net=0;
